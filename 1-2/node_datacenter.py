...
def run_datacenter():
    ...
    notify_monitor('C', '🟢 数据中心存储节点已就绪（SM3+SM4解密增强版）')
    ...
    while True:
        ...
        try:
            ...
            if is_valid:
                ...
                # 3. 核心解密改造（融合小组成员 3 的功能）：
                # 协议层里的 content_bytes 是加密过的，必须先用 SM4 解密还原
                try:
                    ...
                    # 5. 安全持久化落库（此时写入的是完好无损的原始图片二进制）
                    save_filename = "center_storage.jpg"
                    with open(save_filename, "wb") as f:
                        f.write(decrypted_content)

                    print(f"💾 [解密入库成功] 档案已完好还原，成功存储为: {save_filename}")
                    notify_monitor('C', '✅ 档案安全解密，原始图像已成功入库存储')

                except Exception as decrypt_err:
                    print(f"❌ SM4 解密失败！可能是密钥文件不一致或数据损坏: {decrypt_err}")
                    notify_monitor('C', '⚠️ 警告：SM4解密失败，数据拒绝入库')
            else:
